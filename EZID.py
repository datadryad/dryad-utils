...
### This is a temporary version that is borrowed from https://bitbucket.org/mredar/ezid/src/default/
### and updated to only use basic authentication, until that package can be updated for DataCite.
...
class EZIDClient(object):
    ...
    def _get_request(self, request, login=False, basic_auth=True):
        if self._cookie: request.add_header("Cookie", self._cookie)
        try:
            if basic_auth:
                request.add_header("Authorization", "Basic " + base64.b64encode(self._credentials['username'] + ":" + self._credentials['password']))
